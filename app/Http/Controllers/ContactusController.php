...
class ContactusController extends Controller
{
    ...
    public function getAllTickets( Request $request )
    {
		...
		if( $user_role == 1 )// admin
		{
			if($filter == 3)// All ticket
			{
				...
				$data = ContactModel::where('state', $filter)->orderBy('updated_at', 'desc')
							->offset($offset_value)->limit(10)->get();
	            $allData = ContactModel::where('state', $filter)->orderBy('updated_at', 'desc')->count();
				$response['data'] = $data;
				$response['allData'] = $allData;
			}
		}
		else
		{
			if($filter == 3)// All ticket
			{
	            ...
	            $response['allData'] = ContactModel::where('user_id', $user_id)
    	   		...
    	   		$response['data'] = ContactModel::where('user_id', $user_id)->where('state', $filter)
	    	   									->orderBy('updated_at', 'desc')->offset($offset_value)
	                        					->limit(10)->get();
	            $response['allData'] = ContactModel::where('user_id', $user_id)->where('state', $filter)
	    	   									->orderBy('updated_at', 'desc')->count();
    	   	}
		}
    	// response 
    ...
    public function updateTicketReadable(Request $request)
    {
    	// get input data
    	$user = JWTAuth::parseToken()->authenticate();
    	$body 		= $request->all();
    	...
    	if(ContactModel::where('ticket_id', $ticket_id)->exists())
    	{
    		ContactModel::where('ticket_id', $ticket_id)->update(['readable' => $readable]);
    		$response['status'] = 1;	
			$response['message'] = trans('appmessages.ticket_update_success');
		}
		else

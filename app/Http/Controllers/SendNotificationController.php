...
class SendNotificationController extends Controller
{
	...
	public function save_firebase_app_token( Request $request )
	{
		...
		{
			$response['status'] = 1;
			$response['message'] = "This device is already existing.";
		}
		else{
			$firebase = new NotificationFirebaseModel;
			$firebase->app_id	 	  = $body['app_id'];
			$firebase->platform 	  = $body['platform'];
			$firebase->device_token	  = $body['device_token'];
			$firebase->select_channel = "-2";
	...
	//gic 05.26 notification selected channel save
	public function save_selected_channel( Request $request )
	{
		$body = $request->all();
		unset( $body['token']);
		if($body['platform'] == "android"){
		...
		if(NotificationFirebaseModel::where('app_id', $body['app_id'])
									...
									->exists())
		{
			NotificationFirebaseModel::where('app_id', $body['app_id'])
									->where('device_token', $body['device_token'])
									->update($body);
			$response['status'] = 1;
			$response['message'] = "This device is already existing and Registed again";
		}
		else{
			$firebase = new NotificationFirebaseModel;
			$firebase->app_id	 	  = $body['app_id'];
			$firebase->platform 	  = $body['platform'];
			$firebase->device_token	  = $body['device_token'];
			$firebase->select_channel = $body['select_channel'];
	...
	public function pushNotificationMsg( Request $request ){
		...
		if($body){
			...
			$channel  = json_decode($body['select_channel']);
			$response = null;
			$device_channel = "";
			$able_send_flag = false;
			$api_keys;
			$channel_kind;
					...
					if($platform == 3){
						if(NotificationFirebaseModel::where('app_id', $app_id)->exists()){
							...
							foreach ($allDeviceToken as $deviceKey => $eachDevice){
								...
								$device_channel = $eachDevice['select_channel'];
								if($channel[0] == -2 || $channel[0] == -1){
									$able_send_flag = true;
								}
								else{
									foreach ($channel as $channelkey => $oneChannel) {
										$able_send_flag = str_contains($device_channel, $oneChannel);
									}
								}

								if($able_send_flag == true){

									$sendData = array("to" => $eachDevice['device_token'], 
										"notification" => array( "title" => $notify_title, 
											"body" => $notify_body,
											"sound" => "enable" ,
											"icon" => "icon.png",
